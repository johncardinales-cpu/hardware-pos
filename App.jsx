...
function php(v) { return new Intl.NumberFormat('en-PH', { style: 'currency', currency: 'PHP', maximumFractionDigits: 0 }).format(Number.isFinite(v) ? v : 0); }
...
function addCart(c, p) { const e = c.find((i) => i.productId === p.id); return e ? c.map((i) => i.productId === p.id ? { ...i, quantity: i.quantity + 1 } : i) : [...c, { productId: p.id, sku: p.sku, name: p.name, quantity: 1, unitPrice: p.price, unitCost: p.cost }]; }
